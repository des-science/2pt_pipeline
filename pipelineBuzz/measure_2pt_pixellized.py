...
def create_destest_yaml(params, cal_type, group, table, select_path, name_dict):
    ...
    destest_dict['e'] = [name_dict.shape_dict['e1'],
                         name_dict.shape_dict['e2']]
    destest_dict['Rg'] = [
        name_dict.shape_dict['m1'], name_dict.shape_dict['m2']]
    destest_dict['extra_params'] = params
...
def load_catalog(pipe_params, cal_type, group, table, select_path, name_dict, inherit=None, return_calibrator=None):
    ...
    params = create_destest_yaml(
        pipe_params, cal_type, group, table, select_path, name_dict)
...
def task_full(ijk):
    ...
    if 1 == 1:
        path_save = global_measure_2_point.params['run_directory'] + \
            '/2pt/{0}_{1}_{2}/'.format(i, j, k)
        ...
        if not os.path.exists(path_save + '_full.pkl'):
            ...
            if k == 2:
                ndd = (np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 0])) * (
                    np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 1]))
                ndr = (np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 0])) * (
                    np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 3]))
                nrd = (np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 2])) * (
                    np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 1]))
                nrr = (np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 2])) * (
                    np.sum(global_measure_2_point.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 3]))
...
def task(ijk):
    ...
    if not os.path.exists(path_save + lla + '.pkl'):
        ...
        pairs_auto = global_measure_2_point.calc_correlation(i, j, k, [
                                                             jc], [jc])
   ...
   # print average
    for ii in range(TOTAL_PHI.shape[0]):
        for jj in range(ii + 1):
            ...
            cov_jck[jj, ii] = (-average[ii] * average[jj] *
                               jk_r + cov_jck[jj, ii]) * fact
   ...
   # print err_jck
    ...
    for i in range(TOTAL_PHI.shape[0]):
        for j in range(TOTAL_PHI.shape[0]):
            corr[i, j] = cov_jck[i, j] / \
                (np.sqrt(cov_jck[i, i] * cov_jck[j, j]))
...
class Measure2Point(PipelineStage):
    ...
    def __init__(self, param_file):
        ...
        self.Dict = importlib.import_module(
            '.' + self.params['dict_file'], 'pipeline')
        ...
        # Added!
        self.source_regions_selector = load_catalog(
            self.params, 'mcal', self.params['gold_regions_group'], self.params['gold_regions_table'], self.params['gold_regions_path'], self.Dict, inherit=self.source_selector)
        self.lens_regions_selector = load_catalog(
            self.params, 'mcal', self.params['lens_regions_group'], self.params['lens_regions_table'], self.params['lens_regions_path'], self.Dict, inherit=self.source_selector)
        self.lens_random_regions_selector = load_catalog(
            self.params, None, self.params['lens_randoms_regions_group'], self.params['lens_randoms_regions_table'], self.params['lens_randoms_regions_path'], self.Dict)

        # a dictionary that takes unsheared,sheared_1p/1m/2p/2m as u-1-2-3-4 to deal with tuples of values returned by get_col()
        self.Dict.ind = self.Dict.index_dict
    ...
    def setup_jaccknife(self):
        ...
        jack_info_tot = dict()
        for cal in cals:
            if cal.params['cal_type'] is None:
            ...
            for i in range(nbin):
                ...
                if cal.params['cal_type'] is None:
                    ...
                    if cal.params['extra_params']['no_gold_match']:
                        ra_lens = self.lens_selector.source.read(self.Dict.lens_dict['ra'])[
                            self.Dict.ind['u']][cal.selector.get_mask()[self.Dict.ind['u']]][mask]
                        dec_lens = self.lens_selector.source.read(self.Dict.lens_dict['dec'])[
                            self.Dict.ind['u']][cal.selector.get_mask()[self.Dict.ind['u']]][mask]

                    else:
                        gmask = cal.selector.get_match()
                        ra_lens = self.gold_selector.source.read(self.Dict.gold_dict['ra'])[
                            self.Dict.ind['u']][gmask][cal.selector.get_mask()[self.Dict.ind['u']]][mask]
                        dec_lens = self.gold_selector.source.read(self.Dict.gold_dict['dec'])[
                            self.Dict.ind['u']][gmask][cal.selector.get_mask()[self.Dict.ind['u']]][mask]

                    region_lenses = self.lens_regions_selector.source.read(
                        'region')[self.Dict.ind['u']][cal.selector.get_mask()[self.Dict.ind['u']]][mask]

                    jack_info.update({'hpix': region_lenses})
                    #jack_info.update({'n_jck' : self.jack_dict_tot['n_jck']})

                    region_lenses_randoms = (self.lens_random_regions_selector.get_col(
                        self.Dict.regions_dict['region'])[self.Dict.ind['u']][rmask])
                    ...
                    region_sources = self.source_regions_selector.get_col(
                        self.Dict.regions_dict['region'])[self.Dict.ind['u']][mask]
                    #jack_info.update({'n_jck' :  self.jack_dict_tot['n_jck']})
        ...
        jack_info_tot.update(
            {'max_distance': np.array(f['regions']['centers']['dist'])})
        jack_info_tot.update(
            {'n_jck': np.array(f['regions']['centers']['number'][0])})
    ...
    def setup_jobs(self, pool):
        ...
        def distance(cnt, FACT_DIST=2):
            ...
            dist = np.array([np.sort([dist_cent(cnt[i], cnt[j]) for i in range(len(cnt))])[
                            1] for j in range(len(cnt))])
            dist = (dist) * FACT_DIST

            max_sep = self.params['tbounds'][1] * \
                1. / 60.  # ASSUMES INPUT IN ARCMIN!
        ...
        def dist_cent(a, b):
            ...
            a = a * todeg
            b = b * todeg
            cos = np.sin(a[1]) * np.sin(b[1]) + np.cos(a[1]) * \
                np.cos(b[1]) * np.cos(a[0] - b[0])
        ...
        a = np.concatenate((np.array([[(i, j) for i in range(
            self.jack_dict_tot['n_jck'])] for j in range(self.jack_dict_tot['n_jck'])])))

        sel = np.array([max([0., (dist_cent(cnt[i], cnt[j]) - (max_dist_region[i] + max_dist_region[j]))])
                        < (self.params['separation_factor'] * max_sep)for (i, j) in a])
        b = a[sel]
        b1 = []
        ...
        for i, j in all_calcs:
            ...
            for pix_ in b1:
                ...
                if (i <= j) & (j < self.zbins) & (self.params['2pt_only'].lower() in [None, 'shear-shear', 'all']):
                    # Only add to list if calculating shear-shear and a valid tomographic pair (doesn't duplicate identical i<j and i>j)
        ...
        if self.params['lens_group'] != 'None':
            ...
            for i, j in all_calcs:
                ...
                if (i <= j) & (j < self.lens_zbins) & (self.params['2pt_only'].lower() in [None, 'pos-pos', 'all']):
                    Npairs.update(
                        {'{0}_{1}'.format(i, j): self.compute_nobj(i, j)})
        ...
        for i in xrange(nbin):
            ...
            if (i < self.zbins) & (self.params['2pt_only'].lower() in ['pos-shear', 'shear-shear', 'all']):
                ra, dec, g1, g2, w = self.build_catalogs_tot(
                    self.source_calibrator, i)
        ...
        if self.params['lens_group'] != 'None':
            for i in xrange(nbin):
                if (i < self.lens_zbins) & (self.params['2pt_only'].lower() in ['pos-pos', 'pos-shear', 'all']):
                    ra, dec, ran_ra, ran_dec, w, down = self.build_catalogs_tot(
                        self.lens_calibrator, i)
                    bins_dict.update(
                        {'lens_{0}'.format(i): [ra, dec, ran_ra, ran_dec, w, down]})
    ...
    def get_weights(self, i):
        ...
        if cal.params['extra_params']['no_gold_match']:
            ...
            gmask = cal.selector.get_match()
            ra = self.gold_selector.source.read(self.Dict.gold_dict['ra'])[
                self.Dict.ind['u']][gmask][cal.selector.get_mask()[self.Dict.ind['u']]][mask]
            dec = self.gold_selector.source.read(self.Dict.gold_dict['dec'])[
                self.Dict.ind['u']][gmask][cal.selector.get_mask()[self.Dict.ind['u']]][mask]
        ...
        if np.sum(rmask) > self.params['ran_factor'] * np.sum(mask):
            ...
            downsample = np.sort(np.random.choice(np.arange(np.sum(
                rmask)), self.params['ran_factor'] * np.sum(mask), replace=False))  # Downsample

        # Load random ra,dec and calculate healpix values
        ran_ra = self.ran_selector.get_col(self.Dict.ran_dict['ra'])[
            self.Dict.ind['u']][rmask][downsample]
        ran_dec = self.ran_selector.get_col(self.Dict.ran_dict['dec'])[
            self.Dict.ind['u']][rmask][downsample]
    ...
    def compute_nobj(self, i, j):
        ...
        Na, Nb, Nra, Nrb = NN(pix_a, w_a), NN(
            pix_b, w_b), NN(pixr_a, w_ra), NN(pixr_b, w_rb)
        jck_N = np.zeros((n_jck, 4))
        jck_N[:, 0] = Na
        jck_N[:, 1] = Nb
        jck_N[:, 2] = Nra
        jck_N[:, 3] = Nrb
        N_A, N_B, N_RA, N_RB = NA - \
            np.array(Na), NB - np.array(Nb), NRA - \
            np.array(Nra), NRB - np.array(Nrb)
    ...
    def get_zbins_R(self, i, cal):
        ...
        if cal.params['cal_type'] is None:  # Lens catalog
            binning = [f['nofz/lens_zbin'][:]]
        else:  # Source catalog
            binning = []

            if cal.params['extra_params']['has_sheared']:
        ...
        if (type(cal) == destest.NoCalib) & (cal.params['cal_type'] is None):
    ...
    def make_jackknife_regions(self):
        ...
        def jaccknife_regions(ra, dec, number_of_regions, label_jck=None, centers_jck=None, v=False):
            ...
            if not v:
                centers_jck = kmeans_radec.kmeans_sample(
                    A, number_of_regions, maxiter=100, tol=1e-05, verbose=0)
        ...
        def distance_calc(path_label, ra_m, dec_m, jk_m, njk, centers):
            ...
            for i in range(njk):
                if len(ra_m[jk_m == i]) == 0 or len(dec_m[jk_m == i]) == 0:
                    ...
                    tree_m = spatial.cKDTree(
                        np.c_[aJx_u[jk_m == i], aJy_u[jk_m == i], aJz_u[jk_m == i]])

                    max_dist_m, index_dist = tree_m.query(
                        [aJx_r, aJy_r, aJz_r], k=len(ra_m[jk_m == i]))

                    ra_new = ra_m[jk_m == i]
                    dec_new = dec_m[jk_m == i]

                    if (len(ra_m[jk_m == i]) == 1):
                        max_dist_region1[i] = dist_cent_2(
                            ra_c, dec_c, ra_new[index_dist], dec_new[index_dist])
                    else:
                        max_dist_region1[i] = dist_cent_2(
                            ra_c, dec_c, ra_new[index_dist[-1]], dec_new[index_dist[-1]])
        ...
        def dist_cent_2(ra1, dec1, ra2, dec2):
            ...
            cos = np.sin(dec1) * np.sin(dec2) + np.cos(dec1) * \
        ...
        for cal in cals:
            ...
            for i in range(nbin):
                ...
                if type(cal) == destest.NoCalib:  # lens catalog
                    ...
                    pix_lens = convert_to_pix_coord(
                        ra_lens, dec_lens, nside=self.params['hpix_nside'])
                    # Calculate if downsampling is possible
                    if np.sum(rmask) > self.params['ran_factor'] * np.sum(mask):
                        # Set fixed random seed to make results reproducible
                        np.random.seed(seed=self.params['random_seed'])
                    ...
                    ra = self.ran_selector.get_col(self.Dict.ran_dict['ra'])[
                        self.Dict.ind['u']][rmask][downsample]
                    dec = self.ran_selector.get_col(self.Dict.ran_dict['dec'])[
                        self.Dict.ind['u']][rmask][downsample]
                    print('make map')
                    pix = convert_to_pix_coord(
                        ra, dec, nside=self.params['hpix_nside'])
                    ...
                    n_map[unique_pix] += np.bincount(idx_rep,
                                                     weights=np.ones(len(pix)))
                    ...
                    dec = self.gold_selector.get_col(self.Dict.gold_dict['dec'])[
                        self.Dict.ind['u']][mask]
                    print('make map')
                    pix_sources = convert_to_pix_coord(
                        ra, dec, nside=self.params['hpix_nside'])
                    unique_pix, idx, idx_rep = np.unique(
                        pix_sources, return_index=True, return_inverse=True)
                    n_map = np.zeros(hp.nside2npix(self.params['hpix_nside']))
                    n_map[unique_pix] += np.bincount(idx_rep,
                                                     weights=np.ones(len(pix_sources)))
                    mask_footprint = n_map != 0.
                ...
                area = np.array(range(hp.nside2npix(self.params['hpix_nside'])))[
                    mask_footprint]
                dec_mask, ra_mask = IndexToDeclRa(
                    area, self.params['hpix_nside'])

                print('create jackknife')
                path_jackknife = self.params['run_directory'] + '/jackknife/jackknife_centers_{0}_{1}.txt'.format(
                    self.params['hpix_nside'], self.params['n_jck_regions'])

                # find/load jackknife centers.
                if os.path.exists(path_jackknife):
                    centers = np.array(np.loadtxt(path_jackknife))
                    centers_tree, hpix, centers = jaccknife_regions(
                        ra_mask, dec_mask, self.params['n_jck_regions'], path_jackknife, centers, v=True)
                else:
                    centers_tree, hpix, centers = jaccknife_regions(
                        ra_mask, dec_mask, self.params['n_jck_regions'], path_jackknife)
                ...
                path_distance = self.params['run_directory'] + '/jackknife/jackknife_distance_{0}_{1}'.format(
                    self.params['hpix_nside'], self.params['n_jck_regions'])
                if not os.path.exists(path_distance + '.pkl'):
                    distance_calc(path_distance, jack_info['ra'], jack_info['dec'],
                                  jack_info['hpix_jackknife'], jack_info['n_jck'], jack_info['centers'])

                # assign galaxy to jackknife regions.
                if type(cal) == destest.NoCalib:
                    _, hpix_data = centers_tree.query(
                        np.array(zip(ra_lens, dec_lens)))
    ...
    def build_catalogs_tot(self, cal, i):
        ...
        if cal.params['cal_type'] is None:  # lens catalog
            ...
            return ra_lens, dec_lens, ran_ra, ran_dec, w, downsample
            ...
            g1 = cal.selector.get_col(self.Dict.shape_dict['e1'])[
                self.Dict.ind['u']][mask]
            print('----------', g1, self.mean_e1[i], R1)
            g1 = (g1 - self.mean_e1[i]) / R1
            g2 = cal.selector.get_col(self.Dict.shape_dict['e2'])[
                self.Dict.ind['u']][mask]
    ...
    def calc_correlation(self, i, j, k, pix1=None, pix2=None, full=False):
        ...
        if k == 0:
            ...
            if not full:
                ...
                mask_jck = np.in1d(jck_fold['hpix'], pix1)
                ra, dec, g1, g2, w = self.bins_dict['shear_{0}'.format(i)][0][mask_jck], self.bins_dict['shear_{0}'.format(i)][1][mask_jck], self.bins_dict['shear_{0}'.format(
                    i)][2][mask_jck], self.bins_dict['shear_{0}'.format(i)][3][mask_jck], self.bins_dict['shear_{0}'.format(i)][4][mask_jck]
            else:
            ...
            if not full:
                ...
                ra, dec, g1, g2, w = self.bins_dict['shear_{0}'.format(j)][0][mask_jck], self.bins_dict['shear_{0}'.format(j)][1][mask_jck], self.bins_dict['shear_{0}'.format(
                    j)][2][mask_jck], self.bins_dict['shear_{0}'.format(j)][3][mask_jck], self.bins_dict['shear_{0}'.format(j)][4][mask_jck]
            else:
                #ra,dec,g1,g2,w= self.build_catalogs_tot(self.source_calibrator,i)
            ...
            if (icat == None) or (jcat == None):
                pairs = [np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(
                    self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins'])]
            else:

                gg = treecorr.GGCorrelation(nbins=self.params['tbins'], min_sep=self.params['tbounds'][self.Dict.ind['u']],
                                            max_sep=self.params['tbounds'][1], sep_units='arcmin', bin_slop=self.params['slop'], num_threads=num_threads)
                gg.process(icat, jcat)
                ggp = gg.xip
                ggm = gg.xim
                normalization = gg.weight

                pairs = [ggp * normalization, ggm * normalization, normalization, gg.npairs, gg.logr *
                         gg.weight, gg.rnom * gg.weight, gg.meanr * gg.weight, gg.meanlogr * gg.weight]
            ...
            if not full:
                ...
                mask_jck_rndm = np.in1d(
                    jck_fold['hpix_randoms'][downsample], pix1)

                ra, dec, ran_ra, ran_dec, w = self.bins_dict['lens_{0}'.format(i)][0][mask_jck], self.bins_dict['lens_{0}'.format(i)][1][mask_jck], self.bins_dict['lens_{0}'.format(
                    i)][2][mask_jck_rndm], self.bins_dict['lens_{0}'.format(i)][3][mask_jck_rndm], self.bins_dict['lens_{0}'.format(i)][4]

            else:
                ra, dec, ran_ra, ran_dec, w, _ = self.bins_dict['lens_{0}'.format(
                    i)]
            ...
            pairs = [np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(
                self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins'])]

            ng = treecorr.NGCorrelation(nbins=self.params['tbins'], min_sep=self.params['tbounds'][self.Dict.ind['u']],
                                        max_sep=self.params['tbounds'][1], sep_units='arcmin', bin_slop=self.params['slop'], num_threads=num_threads)
            rg = treecorr.NGCorrelation(nbins=self.params['tbins'], min_sep=self.params['tbounds'][self.Dict.ind['u']],
                                        max_sep=self.params['tbounds'][1], sep_units='arcmin', bin_slop=self.params['slop'], num_threads=num_threads)
            ...
            if not full:
                ...
                ra, dec, ran_ra, ran_dec, w = self.bins_dict['lens_{0}'.format(i)][0][mask_jck], self.bins_dict['lens_{0}'.format(i)][1][mask_jck], self.bins_dict['lens_{0}'.format(
                    i)][2][mask_jck_rndm], self.bins_dict['lens_{0}'.format(i)][3][mask_jck_rndm], self.bins_dict['lens_{0}'.format(i)][4][mask_jck]
            else:
                ra, dec, ran_ra, ran_dec, w, _ = self.bins_dict['lens_{0}'.format(
                    i)]
            try:
            ...
            if not full:
                ...
                mask_jck_rndm = np.in1d(
                    jck_fold['hpix_randoms'][downsample], pix2)
                ra, dec, ran_ra, ran_dec, w = self.bins_dict['lens_{0}'.format(j)][0][mask_jck], self.bins_dict['lens_{0}'.format(j)][1][mask_jck], self.bins_dict['lens_{0}'.format(
                    j)][2][mask_jck_rndm], self.bins_dict['lens_{0}'.format(j)][3][mask_jck_rndm], self.bins_dict['lens_{0}'.format(j)][4][mask_jck]
            else:
                ra, dec, ran_ra, ran_dec, w, _ = self.bins_dict['lens_{0}'.format(
                    j)]
            try:
            ...
            pairs = [np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(
                self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins']), np.zeros(self.params['tbins'])]

            dd = treecorr.NNCorrelation(nbins=self.params['tbins'], min_sep=self.params['tbounds'][self.Dict.ind['u']],
                                        max_sep=self.params['tbounds'][1], sep_units='arcmin', bin_slop=self.params['slop'], num_threads=num_threads)
            dr = treecorr.NNCorrelation(nbins=self.params['tbins'], min_sep=self.params['tbounds'][self.Dict.ind['u']],
                                        max_sep=self.params['tbounds'][1], sep_units='arcmin', bin_slop=self.params['slop'], num_threads=num_threads)
            rd = treecorr.NNCorrelation(nbins=self.params['tbins'], min_sep=self.params['tbounds'][self.Dict.ind['u']],
                                        max_sep=self.params['tbounds'][1], sep_units='arcmin', bin_slop=self.params['slop'], num_threads=num_threads)
            rr = treecorr.NNCorrelation(nbins=self.params['tbins'], min_sep=self.params['tbounds'][self.Dict.ind['u']],
                                        max_sep=self.params['tbounds'][1], sep_units='arcmin', bin_slop=self.params['slop'], num_threads=num_threads)
    ...
    def write(self):
        ...
        def collect(pairs, n_jck, n_bins, type_corr, i, j):
            shape = (n_jck, n_bins)
            DD_a, DR_a, RD_a, RR_a, mm1_a, mm2_a, mm3_a, mm4_a, mm5_a, mm6_a, mm7_a, mm8_a, mm9_a, mm10_a = np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(
                shape), np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(shape), np.zeros(shape)
            DD, DR, RD, RR, mm1, mm2, mm3, mm4, mm5, mm6, mm7, mm8, mm9, mm10 = np.zeros(n_bins),  np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(
                n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins), np.zeros(n_bins)
            ...
            for n in range(n_bins):
                for jk1 in range(len(pairs[:, 0, 0, n])):
                    DD[n] += (pairs[jk1, 0, 0, n]) + \
                        FACT_0 * (pairs[jk1, 1, 0, n])
                    DR[n] += ((pairs[jk1, 0, 1, n]) +
                              FACT_0 * (pairs[jk1, 1, 1, n]))
                    # print DR, DD, RD
                    RD[n] += ((pairs[jk1, 0, 2, n]) +
                              FACT_0 * (pairs[jk1, 1, 2, n]))
                    try:
                        RR[n] += ((pairs[jk1, 0, 3, n]) +
                                  FACT_0 * (pairs[jk1, 1, 3, n]))
                    except:
                        pass
                    try:
                        mm1[n] += ((pairs[jk1, 0, 4, n]) +
                                   FACT_0 * (pairs[jk1, 1, 4, n]))
                    except:
                        pass
                    try:
                        mm2[n] += ((pairs[jk1, 0, 5, n]) +
                                   FACT_0 * (pairs[jk1, 1, 5, n]))
                    except:
                        pass
                    try:
                        mm3[n] += ((pairs[jk1, 0, 6, n]) +
                                   FACT_0 * (pairs[jk1, 1, 6, n]))
                    except:
                        pass
                    try:
                        mm4[n] += ((pairs[jk1, 0, 7, n]) +
                                   FACT_0 * (pairs[jk1, 1, 7, n]))
                    except:
                        pass
                    try:
                        mm5[n] += ((pairs[jk1, 0, 8, n]) +
                                   FACT_0 * (pairs[jk1, 1, 8, n]))
                    except:
                        pass
                    try:
                        mm6[n] += ((pairs[jk1, 0, 9, n]) +
                                   FACT_0 * (pairs[jk1, 1, 9, n]))
                    except:
                        pass
                    try:
                        mm7[n] += ((pairs[jk1, 0, 10, n]) +
                                   FACT_0 * (pairs[jk1, 1, 10, n]))
                    except:
                        pass
                    try:
                        mm8[n] += ((pairs[jk1, 0, 11, n]) +
                                   FACT_0 * (pairs[jk1, 1, 11, n]))
                    except:
                        pass
                    try:
                        mm9[n] += ((pairs[jk1, 0, 12, n]) +
                                   FACT_0 * (pairs[jk1, 1, 12, n]))
                    except:
                        pass
                    try:
                        mm10[n] += ((pairs[jk1, 0, 13, n]) +
                                    FACT_0 * (pairs[jk1, 1, 13, n]))
                    except:
                        pass
            for n in range(n_bins):
                for jk1 in range(len(pairs[:, 0, 0, n])):
                    DD_a[jk1, n] = DD[n] - (pairs[jk1, 0, 0, n]) - \
                        fact * FACT_0 * (pairs[jk1, 1, 0, n])
                    DR_a[jk1, n] = DR[n] - (pairs[jk1, 0, 1, n]) - \
                        fact * FACT_0 * (pairs[jk1, 1, 1, n])
                    RD_a[jk1, n] = RD[n] - (pairs[jk1, 0, 2, n]) - \
                        fact * FACT_0 * (pairs[jk1, 1, 2, n])
                    try:
                        RR_a[jk1, n] = RR[n] - (pairs[jk1, 0, 3, n]) - \
                            fact * FACT_0 * (pairs[jk1, 1, 3, n])
                    except:
                        pass
                    try:
                        mm1_a[jk1, n] = mm1[n] - \
                            (pairs[jk1, 0, 4, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 4, n])
                    except:
                        pass
                    try:
                        mm2_a[jk1, n] = mm2[n] - \
                            (pairs[jk1, 0, 5, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 5, n])
                    except:
                        pass
                    try:
                        mm3_a[jk1, n] = mm3[n] - \
                            (pairs[jk1, 0, 6, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 6, n])
                    except:
                        pass
                    try:
                        mm4_a[jk1, n] = mm4[n] - \
                            (pairs[jk1, 0, 7, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 7, n])
                    except:
                        pass
                    try:
                        mm5_a[jk1, n] = mm5[n] - \
                            (pairs[jk1, 0, 8, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 8, n])
                    except:
                        pass
                    try:
                        mm6_a[jk1, n] = mm6[n] - \
                            (pairs[jk1, 0, 9, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 9, n])
                    except:
                        pass
                    try:
                        mm7_a[jk1, n] = mm7[n] - \
                            (pairs[jk1, 0, 10, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 10, n])
                    except:
                        pass
                    try:
                        mm8_a[jk1, n] = mm8[n] - \
                            (pairs[jk1, 0, 11, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 11, n])
                    except:
                        pass
                    try:
                        mm9_a[jk1, n] = mm9[n] - \
                            (pairs[jk1, 0, 12, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 12, n])
                    except:
                        pass
                    try:
                        mm10_a[jk1, n] = mm10[n] - \
                            (pairs[jk1, 0, 13, n]) - fact * \
                            FACT_0 * (pairs[jk1, 1, 13, n])
                    except:
                        pass
                    # print RD_a
            ...
            if type_corr == 'pos_pos':
                ...
                xi[masku] = (DD[masku] - DR[masku] * norm[1] - RD[masku]
                             * norm[2] + RR[masku] * norm[3]) / (RR[masku] * norm[3])

                for jk in range(self.jack_dict_tot['n_jck']):
                    ndd = (np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 0]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 0]) * (
                        np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 1]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 1])
                    ndr = (np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 0]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 0]) * (
                        np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 3]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 3])
                    nrd = (np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 2]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 2]) * (
                        np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 1]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 1])
                    nrr = (np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 2]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 2]) * (
                        np.sum(self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][:, 3]) - self.Npairs['{0}_{1}'.format(i, j)]['jck_N'][jk, 3])
                    norm = np.array([1., ndd / ndr, ndd / nrd, ndd / nrr])

                    masku = RR_a[jk, :] != 0.
                    xi_j[jk, masku] = (DD_a[jk, masku] - DR_a[jk, masku] * norm[1] - RD_a[jk, masku]
                                       * norm[2] + RR_a[jk, masku] * norm[3]) / (RR_a[jk, masku] * norm[3])
        ...
        for i, j in self.all_calcs:
            ...
            if (i <= j) & (j < self.zbins) & (self.params['2pt_only'].lower() in [None, 'shear-shear', 'all']):
                ...
                if (self.params['region_mode'] == 'pixellized') or (self.params['region_mode'] == 'both'):
                    ...
                    path = self.params['run_directory'] + \
                        '/2pt/{0}_{1}_{2}/'.format(i, j, 0)
                    ...
                    xip, xim, xip_j, xim_j, npairs, weight, logr, rnom, meanr, meanlogr = collect(
                        pairs_ring, self.jack_dict_tot['n_jck'], self.params['tbins'], 'shear_shear', i, j)
                    cov_xip = covariance_jck(
                        xip_j.T, self.jack_dict_tot['n_jck'], 'jackknife')
                    cov_xim = covariance_jck(
                        xim_j.T, self.jack_dict_tot['n_jck'], 'jackknife')
                    files = dict()
                ...
                if (self.params['region_mode'] == 'full') or (self.params['region_mode'] == 'both'):
                    path = self.params['run_directory'] + \
                        '/2pt/{0}_{1}_{2}/_full'.format(i, j, 0)
                    if os.path.exists(path + '.pkl'):
                        ...
                        output_shear_shear_full.update(
                            {'{0}_{1}'.format(i, j): muted})
            ...
            for i, j in self.all_calcs:
                if (i < self.lens_zbins) & (j < self.zbins) & (self.params['2pt_only'].lower() in [None, 'pos-shear', 'all']):
                    ...
                    if (self.params['region_mode'] == 'pixellized') or (self.params['region_mode'] == 'both'):
                        ...
                        pairs_ring = np.array(pairs_ring)
                        self.pairs_ring = pairs_ring
                        xi, xi_j, xir, xir_j, xi_im, xi_im_j, xi_imr, xi_imr_j, npairs, npairs_im, weight, weight_im, gammat_mr, gammat_mr_im, gammat_mr_j, gammat_mr_im_j, logr, rnom, meanr, meanlogr = collect(
                            pairs_ring, self.jack_dict_tot['n_jck'], self.params['tbins'], 'shear_pos', i, j)

                        cov_xi = covariance_jck(
                            xi_j.T, self.jack_dict_tot['n_jck'], 'jackknife')
                        cov_xir = covariance_jck(
                            xir_j.T, self.jack_dict_tot['n_jck'], 'jackknife')

                        cov_xi_im = covariance_jck(
                            xi_im_j.T, self.jack_dict_tot['n_jck'], 'jackknife')
                        cov_xi_imr = covariance_jck(
                            xi_imr_j.T, self.jack_dict_tot['n_jck'], 'jackknife')

                        cov_xi_mr = covariance_jck(
                            xi_j.T - xir_j.T, self.jack_dict_tot['n_jck'], 'jackknife')
                        cov_xi_mr_im = covariance_jck(
                            xi_im_j.T - xi_imr_j.T, self.jack_dict_tot['n_jck'], 'jackknife')
                        ...
                        save_obj(path + '/results', files)
                        output_shear_pos.update(
                            {'{0}_{1}'.format(i, j): files})

                    if (self.params['region_mode'] == 'full') or (self.params['region_mode'] == 'both'):
                        path = self.params['run_directory'] + \
                            '/2pt/{0}_{1}_{2}/_full'.format(i, j, 1)
                        if os.path.exists(path + '.pkl'):
                            ...
                            muted.update(
                                {'gammat_compens_im': mute[9] / mute[2]})
            ...
            for i, j in self.all_calcs:
                if (i <= j) & (j < self.lens_zbins) & (self.params['2pt_only'].lower() in [None, 'pos-pos', 'all']):
                    ...
                    if (self.params['region_mode'] == 'pixellized') or (self.params['region_mode'] == 'both'):
                        ...
                        pairs_ring = np.array(pairs_ring)
                        self.pairs_ring = pairs_ring
                        xi, xi_j, npairs, weight, logr, rnom, meanr, meanlogr = collect(
                            pairs_ring, self.jack_dict_tot['n_jck'], self.params['tbins'], 'pos_pos', i, j)
                    ...
                    if (self.params['region_mode'] == 'full') or (self.params['region_mode'] == 'both'):
                        path = self.params['run_directory'] + \
                            '/2pt/{0}_{1}_{2}/_full'.format(i, j, 2)
                        if os.path.exists(path + '.pkl'):
                            mute = load_obj(path)
                            xi = (mute[0] - mute[1] -
                                  mute[2] + mute[3]) / mute[3]
                            muted = dict()
                            muted.update({'w': xi})
                            ...
                            output_pos_pos_full.update(
                                {'{0}_{1}'.format(i, j): muted})

        self.output_shear_shear = output_shear_shear
        save_obj(self.params['run_directory'] +
                 '/2pt/shear_shear_pixellized', self.output_shear_shear)

        self.output_shear_pos = output_shear_pos
        save_obj(self.params['run_directory'] +
                 '/2pt/shear_pos_pixellized', self.output_shear_pos)

        self.output_pos_pos = output_pos_pos
        save_obj(self.params['run_directory'] +
                 '/2pt/pos_pos_pixellized', self.output_pos_pos)

        # add full if present
        self.output_shear_shear_full = output_shear_shear_full
        save_obj(self.params['run_directory'] +
                 '/2pt/shear_shear_full', self.output_shear_shear_full)

        self.output_shear_pos_full = output_shear_pos_full
        save_obj(self.params['run_directory'] +
                 '/2pt/shear_pos_full', self.output_shear_pos_full)

        self.output_pos_pos_full = output_pos_pos_full
        save_obj(self.params['run_directory'] +
                 '/2pt/pos_pos_full', self.output_pos_pos_full)
