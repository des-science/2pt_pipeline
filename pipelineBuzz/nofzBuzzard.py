...
class nofz(PipelineStage):
    ...
    def __init__(self, param_file):
        ...
        self.source_selector, self.source_calibrator = load_catalog(self.params, None , self.params['source_group'], self.params['source_table'], self.params['source_path'], self.Dict, return_calibrator=destest.NoCalib)
        ...
        self.gold_selector = load_catalog(self.params, None, self.params['gold_group'], self.params['gold_table'], self.params['gold_path'], self.Dict, inherit=self.source_selector)
        self.pz_selector   = load_catalog(self.params, None, self.params['pz_group'], self.params['pz_table'], self.params['pz_path'], self.Dict,   inherit=self.source_selector)
        ...
        self.source_regions_selector = load_catalog(self.params, None, self.params['gold_regions_group'], self.params['gold_regions_table'], self.params['gold_regions_path'], self.Dict, inherit=self.source_selector)
        self.lens_regions_selector = load_catalog(self.params, None, self.params['lens_regions_group'], self.params['lens_regions_table'], self.params['lens_regions_path'], self.Dict,inherit=self.source_selector)
            ...
            for i in range(zbins):
                ...
                if shape:
                    if self.params['has_sheared']:
                        ...
                        weight_ = np.ones(np.shape(stack_col[mask])) #1. #np.ones_like(self.Dict.shape_dict['e1'])
                        #print(['e1'])
    ...
    def get_sige_neff(self, zbin, tomobins):
        ...
        for i in range(tomobins):
            ...
            if self.params['has_sheared']:
                ...
                mask    = (zbin[0] == i)
                e1  = self.source_selector.get_col(self.Dict.shape_dict['e1'], nosheared=True)[self.Dict.ind['u']][mask]
                e2  = self.source_selector.get_col(self.Dict.shape_dict['e2'], nosheared=True)[self.Dict.ind['u']][mask]   
                w = 1.0
                s = 1.0 
                var = 0.0
                #raise ParamError('Not updated to support non-metacal catalogs.')
