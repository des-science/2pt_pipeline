...
class nofz(PipelineStage):
    ...
    def __init__(self, param_file):
        ...
        if self.params['has_sheared']:  # UNDER EDIT AA
        ...
        if self.params['pdf_type'] != 'pdf':
            ...
            self.z = np.linspace(
                0., self.params['nzmax'], self.params['nzbins'] + 1)
            self.z = (self.z[1:] + self.z[:-1]) / 2. + \
                1e-4  # 1e-4 for buzzard redshift files
        ...
        if hasattr(self.params['zbins'], "__len__"):
            ...
            self.binedges = self.find_bin_edges(self.pz_selector.get_col(
                self.Dict.pz_dict['pzbin']), self.tomobins, w=self.shape['weight'][self.mask])
        ...
        if self.params['lens_group'] != 'None':
            if hasattr(self.params['lens_zbins'], "__len__"):
                ...
                self.lens_binedges = self.find_bin_edges(
                    self.lens_pz['pzbin'], self.lens_tomobins, w=self.lens['weight'])
    ...
    def run(self):
        ...
        pzstack = self.pz_selector.get_col(self.Dict.pz_dict['pzstack'])[
            self.Dict.ind['u']]
        ...
        for zbin_, zname in tuple(zip(zbin, ['zbin', 'zbin_1p', 'zbin_1m', 'zbin_2p', 'zbin_2m'])):
            f.create_dataset('nofz/' + zname, maxshape=(2 * len(zbin_),),
                             shape=(len(zbin_),), dtype=zbin_.dtype, chunks=(len(zbin_) / 10,))
            f['nofz/' + zname][:] = zbin_

        # Get the lens PZ binning and stacking arrays and weights
        pzbin = self.lens_selector.get_col(self.Dict.lens_pz_dict['pzbin'])
        pzstack = self.lens_selector.get_col(self.Dict.lens_pz_dict['pzstack'])[
            self.Dict.ind['u']]
        weight = self.lens_calibrator.calibrate(
            self.Dict.lens_pz_dict['weight'], weight_only=True)
        ...
        if self.params['lens_group'] != 'None':
            ...
            f.create_dataset('nofz/lens_zbin', maxshape=(len(lens_zbin),), shape=(
                len(lens_zbin),), dtype=lens_zbin.dtype, chunks=(len(lens_zbin) / 10,))
            f['nofz/lens_zbin'][:] = lens_zbin

            # Calculate and write random tomographic binning indicies to file for use later in the pipeline
            pzbin = self.ran_selector.get_col(self.Dict.ran_dict['ranbincol'])[
                self.Dict.ind['u']]
            ran_binning = np.digitize(
                pzbin, self.lens_binedges, right=True) - 1

            f.create_dataset('nofz/ran_zbin', maxshape=(len(ran_binning),), shape=(
                len(ran_binning),), dtype=ran_binning.dtype, chunks=(len(ran_binning) / 10,))
    ...
    def write(self):
        ...
        kernels = [nz_source]
        np.savetxt(self.output_path("nz_source_txt"),
                   np.vstack((self.binlow, self.nofz)).T)
    ...
    def build_nofz_bins(self, zbins, edge, bin_col, stack_col, pdf_type, weight, shape=False):
        ...
        if (pdf_type == 'sample') | (pdf_type == 'rm'):
            ...
            if pdf_type == 'rm':
                ...
                stack_col = np.random.normal(stack_col, self.lens_selector.get_col(
                    self.Dict.lens_pz_dict['pzerr'])[self.Dict.ind['u']])
            ...
            for i in range(zbins):
                ...
                if shape:
                    if self.params['has_sheared']:
                        ...
                        weight_ = self.source_calibrator.calibrate(self.Dict.shape_dict['e1'], mask=[
                                                                   mask], return_wRg=True)  # This returns an array of (Rg1+Rg2)/2*w for weighting the n(z)
                        print('weight', weight_)

                    else:
                        print("not a metacal catalogue")
                        # 1. #np.ones_like(self.Dict.shape_dict['e1'])
                        weight_ = np.ones(np.shape(stack_col[mask]))
                        # print(['e1'])
                        print(np.shape(stack_col[mask]))
                ...
                if np.isscalar(weight_):
                    ...
                    nofz[i, :], b = np.histogram(
                        stack_col[mask], bins=np.append(self.binlow, self.binhigh[-1]))
                else:
                    print(weight_)
                    print(np.shape(weight_))
                    nofz[i, :], b = np.histogram(stack_col[mask], bins=np.append(
                        self.binlow, self.binhigh[-1]), weights=weight_)
    ...
    def get_lens_neff(self, zbin, tomobins, weight):
        ...
        for i in range(tomobins):
            ...
            if np.isscalar(weight):
                ...
                self.lens_neff.append(np.asscalar(
                    np.sum(mask) / (self.area * 60.**2)))
    ...
    def get_sige_neff(self, zbin, tomobins):
        ...
        for i in range(tomobins):
            ...
            if self.params['has_sheared']:
                ...
                R, c, w = self.source_calibrator.calibrate(self.Dict.shape_dict['e1'], mask=[
                                                           mask, mask_1p, mask_1m, mask_2p, mask_2m])
                print('response', R)
                R, c, w = self.source_calibrator.calibrate(self.Dict.shape_dict['e2'], mask=[
                                                           mask, mask_1p, mask_1m, mask_2p, mask_2m])
                print('response', R)
                if type(w) is list:
                    w = w[0]
                ...
                e2 = self.source_selector.get_col(self.Dict.shape_dict['e2'],
                                                  nosheared=True)[self.Dict.ind['u']][mask]
                s = R
                var = self.source_selector.get_col(self.Dict.shape_dict['cov00'], nosheared=True)[
                    self.Dict.ind['u']][mask] + self.source_selector.get_col(self.Dict.shape_dict['cov11'], nosheared=True)[self.Dict.ind['u']][mask]
...
def find_git_hash():
    try:
        ...
        head = subprocess.check_output(
            "cd {0}; git show-ref HEADS".format(dirname), shell=True)
    except subprocess.CalledProcessError:
        head = "UNKNOWN"
        warnings.warn(
            "Unable to find git repository commit ID in {}".format(dirname))
    return head.split()[0]
